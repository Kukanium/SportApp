...
import './Styles/СMatchesStyle.css';
import {Link} from "react-router-dom";
import {compareAsc} from 'date-fns'

export default class CompetitionMatches extends Component {            // Страница со списком матчей определённого, по id, соревнования
    ...
    componentDidMount() {
        ...
        fetch(`http://api.football-data.org/v2/competitions/${this.props.match.params.id}/matches/`, { headers }) 
            ...
            return (
                <div className="List">
                    <div className="MatchFilter">
                        ...
                        <Link to={`/CompetitionMatches/${this.props.match.params.id}/?q=${searchTermStart}/${searchTermEnd}`} >
                            <button className="FilterButton"> Отсортировать </button>
                        </Link>                        
                    </div>
                    <div className='ListOfCompMatches'>
                        {matches.filter((match) => {
                           ...
                           if (res1 == 1  && res2 == -1 ){  //Сравнение даты из текущего элемента с датой заданной в поиске
                            ...
                            <div key={match.id} className='CompMatchCard' >
